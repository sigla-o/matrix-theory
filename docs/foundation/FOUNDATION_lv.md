...
MT kvantitatīvais pamats ir definēts MATHEMATICS_lv.md. ID sistēma (ID_GRADIENT_lv.md) nodrošina organizācijas līmeņu klasifikāciju. Šeit ir apkopoti galvenie operatori, lielumi un ID līmeņi, kas tiek izmantoti visos pārējos dokumentos:
...
| \( \Phi(\mathbf{x},\mathbf{y}) \) | \( \mathcal{L} \times \mathcal{L} \to \mathbb{R}^+ \) | TE plūsmas lauks | ID0 (pamats) |
...
| \( \gamma \) | \( \lambda_{\text{ID1}}/R_{L1} \approx 0.18 \) | G mainības konstante | — |
| \( C \) | \( 2\pi R_{L1}/\lambda_{\text{ID1}} \approx 35.325 \) | CMB projekcijas konstante | — |

---

## 3. ID SISTĒMAS PAMATLĪMEŅI
...
| ID0 | — | 10⁻³⁵ | Matrica (režģis, TE plūsmas pamats) |
...
**Sekas:**
1. Lielāka telpa rada **jaunu Qn TE plūsmas struktūru**.
...
## 6. L1 ZONA UN CMB — KVANTITATĪVAIS MODELIS
...
|----------|-------------|-------------------|------------------|---------------|
| Smalkās struktūras konstante | \( \alpha = \frac{49 G_0}{24\pi \hbar c} \) | \( \alpha_{\text{MT}} \approx 0.0073 \) | Precīzijas spektroskopija | ID1 |
| G mainība galaktiku centros | \( G(r) = G_0 \cdot (1 + 0.18 \cdot \rho_{\mathcal{V}}(r)/\rho_{\text{H0}}) \) | \( G(0)/G_0 \approx 1.50 \) | Zvaigžņu orbītas (GRAVITY interferometrs) | ID2 |
| 6. CMB pīķis | \( \ell_6 = 35.325 \times 47 \) | \( \ell_6 \approx 1660 \) | CMB-S4, Simons Observatory | ID0 |
| 7. CMB pīķis | \( \ell_7 = 35.325 \times 55 \) | \( \ell_7 \approx 1943 \) | CMB-S4, Simons Observatory | ID0 |
| ID atkarība no \( \rho_{\mathcal{V}} \) | \( \text{ID} = 2.0 + \log_{2.5}(n) + 0.05 \cdot \rho_{\mathcal{V}}/\rho_{\text{H0}} \) | Melno caurumu masas korelācija ar galaktikas tipu | Melno caurumu masu mērījumi | ID4 |
| ε₀ atkarība no \( \rho_{\mathcal{V}} \) | \( \varepsilon_0(\rho_{\mathcal{V}}) = \varepsilon_0^{(0)} \cdot (1 + \gamma_\varepsilon \cdot \rho_{\mathcal{V}}/\rho_{\text{H0}}) \) | — | Kvantu metroloģija | ID1 |
| Apziņas intensitāte anestēzijā | \( \mathcal{C} = \int \| \mathcal{P}_{L1} - \Phi \|^2 \) | \( \mathcal{C} \) palielinās | EEG/fMRI kvantitatīvs mērījums | ID1, ID2 |
