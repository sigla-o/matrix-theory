...
A channel is the set of shortest TE flow paths within the H0 matrix whose instantaneous geometry and flow parameters are determined by the cyclic pressure and rarefaction zones generated by the rotation of the H+1 matrix.

The rotation of the H+1 matrix outside the H0 sphere produces cyclic pressure/rarefaction zones around the entire H0 sphere. Each zone contains approximately 10^20 H0 matrix points. These zones determine not only the instantaneous direction of H0 channels but also the modulation of FV flow across groups of 10^20 channels.

Each FV retains its own flow inertia; therefore, every flow is unique and cannot be completely reduced to the average state of surrounding flows.

The **minimum channel length** is the flow path between two successive H0 matrix channel overlap points.

The **maximum channel length** is one complete rotation cycle of a matrix point, which may repeat for an indefinite number of cycles as long as a pressure difference remains in at least one channel.
